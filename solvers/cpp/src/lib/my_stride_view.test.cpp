...
TEST(MyStrideView, BetweenPipes) {
  ...
  const auto processed = data | views::drop(1) | MyStride(3) | views::take(3) |
...
TEST(MyStrideView, Empty) {
  ...
  const auto processed = empty | MyStride(2) | ranges::to<std::vector>();
  EXPECT_TRUE(processed.empty());
}

TEST(MyStrideView, One) {
  constexpr std::array<int, 1> one{1};
  const auto processed = one | MyStride(2) | ranges::to<std::vector>();
...
TEST(MyStrideView, ZeroStride) {
  ...
  const auto processed = data | MyStride(0) | ranges::to<std::vector>();
  EXPECT_TRUE(processed.empty());
}
