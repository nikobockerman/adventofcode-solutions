...
consteval auto getOperators() -> std::span<const Operator> {
  if constexpr (allowConcatenateOperator) {
    constexpr static auto result =
      std::array{Operator::Multiply, Operator::Plus, Operator::Concatenate};
    return std::span(result);
  }
  constexpr static auto result = std::array{Operator::Multiply, Operator::Plus};
  return std::span(result);
}
